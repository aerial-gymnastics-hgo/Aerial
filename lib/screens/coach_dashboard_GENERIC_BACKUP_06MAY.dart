...
import '../models/trial_request_model.dart';
import 'dart:async';
...
class _CoachDashboardState extends State<CoachDashboard> {
  ...
  List<User> _students = [];
  List<TrialRequest> _trialRequests = [];
  List<GymGroup> _allGroups = [];

  StreamSubscription? _rotSub, _evSub, _mesoSub, _assSub, _stuSub, _trialSub, _groupsSub;
  ...
  void initState() {
    ...
    _trialSub = FirestoreService.instance.getTrialRequests().listen((data) {
      if (mounted) setState(() => _trialRequests = data);
    });
  ...
  void _updateCurrentRotation() {
    ...
    try {
      currentSlot = todayRotations.firstWhere(
        (slot) {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildTrialRequestAlert(primaryColor),
  ...
  void _showNoticesModal(BuildContext context, Color primaryColor) {
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        // Usar StatefulBuilder para actualizar el modal al descartar
  ...
  Widget _buildStrategicFocusSection(Color primaryColor) {
    final nextComp = _events.where((e) => e.type == 'event').firstOrNull ??
        SystemEvent(
            ...
            targetRole: 'all',
            type: 'event');
    final daysLeft = nextComp.date
        .difference(DateTime.now())
        .inDays
        .abs(); // Evitar netagivos si es dummy

    // Si hay grupo activo, mostrar enfoque específico
          ...
          _buildGlassContainer(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                value: (selectedGroup == 'auto' || _allGroups.any((g) => g.id == selectedGroup)) ? selectedGroup : 'auto',
                isExpanded: true,
  ...
  // ── Vista principal: Timeline de tarjetas coloreadas ─────────────────────
      ...
      child: Column(
        ...
        children: [
          ...
          ...myBlocks.asMap().entries.map((entry) {
            ...
            // Verificar si es el bloque actual (hora activa)
            final now = DateTime.now();
  ...
  Widget _buildTrialRequestAlert(Color primaryColor) {
    // 1. Obtener grupos del coach hoy
    final myGroupIds = _assignments
        .where((e) =>
            e.coachId == currentUser.id && e.dayOfWeek == _getCurrentDayName())
        .map((e) => e.groupId)
        .toSet();

    // 2. Buscar solicitudes agendadas para hoy en esos grupos
    final now = DateTime.now();
    final todayTrials = _trialRequests.where((r) {
      if (r.status != TrialStatus.scheduled ||
          r.scheduledDate == null ||
          r.assignedGroupId == null) return false;

      final isToday = r.scheduledDate!.year == now.year &&
          r.scheduledDate!.month == now.month &&
          r.scheduledDate!.day == now.day;

      return isToday && myGroupIds.contains(r.assignedGroupId);
    }).toList();

    if (todayTrials.isEmpty) return const SizedBox.shrink();

    return Container(
      margin: const EdgeInsets.fromLTRB(16, 16, 16, 0),
      child: InkWell(
        onTap: () => _showTrialRequestsDetail(todayTrials),
        child: Container(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Tienes ${todayTrials.length} prospecto(s) nuevos en tus grupos.',
                      style: GoogleFonts.poppins(
  ...
  void _showTrialRequestsDetail(List<TrialRequest> trials) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(
                            '${trial.childName} (${trial.childAge} años)',
                            style: GoogleFonts.montserrat(
                                fontWeight: FontWeight.bold,
                                color: Colors.white,
                                fontSize: 16),
                          ...
                          Container(
                            ...
                            child: Text(trial.experienceLevel,
                                style: const TextStyle(
                                    color: Colors.amber,
                                    fontSize: 10,
                                    fontWeight: FontWeight.bold)),
                      ...
                      if (trial.medicalNotes != null) ...[
                        ...
                        const Text('Sin restricciones médicas reportadas.',
                            style:
